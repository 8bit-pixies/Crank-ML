...
class NeuralDecisionBoostingClassifier(nn.Module):
    def __init__(
        self,
        n_features: int,
        n_estimators: int,
    ...
    def train_forward(self, X, y):
        ...
        for indx in range(self.n_estimators):
            ...
            model_loss = F.cross_entropy(y_hat, y, reduction="none") * weight
            ...
            sign_output = 1 - ((((torch.argmax(y_hat, axis=1) == torch.argmax(y, axis=1)) * 0.5) - 1) * 2)
