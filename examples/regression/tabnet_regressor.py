...
from crank_ml.tabnet.tabnet_regressor import TabNetRegressor


class RegressorExample(pl.LightningModule):
    def __init__(self):
        super().__init__()
        self.model = TabNetRegressor(100)
    ...
    def _shared_eval_step(self, batch, batch_idx):
        x, y = batch
        y_hat, loss_tabnet = self.model(x)
        loss_func = nn.MSELoss()
        loss = loss_func(y_hat, y)
        return loss + loss_tabnet
