...
from crank_ml.tree.neural_decision_boosting_classifier import NeuralDecisionBoostingClassifier


class IrisClassifier(pl.LightningModule):
    def __init__(self):
        super().__init__()
        self.model = NeuralDecisionBoostingClassifier(
            4,
            100,
            n_classes=3,
        )
    ...
    def _shared_eval_step(self, batch, batch_idx):
        x, y = batch
        loss = self.model.train_forward(x, y)
        y_hat = self.model(x)
